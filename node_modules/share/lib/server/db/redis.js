...
module.exports = RedisDb = function(options) {
  ...
  //client = redis.createSocketClient(process.env.OPENSHIFT_GEAR_DIR + 'tmp/redis.sock', options.redisOptions);
  
  if (options.auth && typeof options.auth === "string") {
    client.auth(__indexOf.call(options.auth, ":") >= 0 ? options.auth.split(":").pop() : options.auth);
  }
  if (options.testing) {
    client.select(15);
  }
